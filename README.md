...
### High-level flow
    ...
    ```mermaid
     graph TD
         ...
         classDef api fill:#efebe9,stroke:#795548,stroke-width:2px,color:#000;
         ...
         classDef worker fill:#f3e5f5,stroke:#9c27b0,stroke-width:2px,color:#000;
         classDef output fill:#eceff1,stroke:#607d8b,stroke-width:2px,color:#000;

         %% Nodes with Design Icons
         Client["📱 Client"]:::client
         OrdersAPI["⚙️ Orders API"]:::api
         Postgres[("🐘 PostgreSQL DB")]:::db
    
         StockEvents["📋 stock_events (Table/Queue)"]:::queue
         OrderEvents["📋 order_events (Table/Queue)"]:::queue
    
         StockWorker["🤖 Stock Worker"]:::worker
         EventsConsumer["🤖 Events Consumer"]:::worker
    
         ProductsTable[("📦 Products Table")]:::db
         IntegrationOut["🚀 Integration Output"]:::output

         %% Flow/Connections
         Client --> OrdersAPI
         OrdersAPI --> Postgres
    
         Postgres --> StockEvents
         Postgres --> OrderEvents
    
         StockEvents --> StockWorker
         StockWorker --> ProductsTable
    
         OrderEvents --> EventsConsumer
         EventsConsumer --> IntegrationOut
    ```
